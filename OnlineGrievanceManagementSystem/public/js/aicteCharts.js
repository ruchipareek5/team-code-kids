...
$('#graphs').ready(function(){
    ...
    $(document).on('change', '#stateName', function() {
        var stateName = $('#stateName').val();
        loadStateChart(stateName);
      });
      loadStateChart(stateName);      
      function loadStateChart(stateName){
          
        $.getJSON(baseUrl+'grievance/aicte/chart/department/'+stateName, function(data) {
            if(data[0].length==0){
                alertShow('error','No recorded grievances found')
            }
    // graph color options
